...
\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA DE DIAGNÓSTICO – MAPEO DE SEÑALES DEL PLC ORIGINAL, PUNTOS DE BIFURCACIÓN DEL BYPASS E INSTRUMENTACIÓN AÑADIDA]}
...
\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA DE FLUJO DE LA METODOLOGÍA – FASES, ENTREGABLES Y CORRESPONDENCIA DISEÑO-VALIDACIÓN (V-MODEL)]}
...
\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA DE FLUJO DE INFORMACIÓN – ESP32 → MQTT BROKER → NODE-RED → WEBOTS → SCADA]}
...
\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA ESQUEMÁTICO DEL PANEL SCADA – UBICACIÓN DE ESP32, FUENTE, RELÉS, TERMINALES Y PROTECCIONES]}
...
\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA DE CONEXIÓN DE RELÉS Y ACTUADORES – MAPEO ESP32 → OPTOACOPLADORES → RELÉS → ACTUADORES, BYPASS CON PLC ORIGINAL]}
\caption{Conexión de relés y esquema de bypass.}
\label{fig:conex_relays}
\end{figure}

\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA DE INSTRUMENTACIÓN DE SENSORES – CONEXIÓN DE INFRARROJOS, ULTRASÓNICO Y TRANSDUCTOR DE PRESIÓN]}
\caption{Instrumentación de sensores con acondicionamiento de señales.}
\label{fig:instr_sens}
\end{figure}
...
El gemelo digital reproduce la volumetría real de la dosificadora a partir de modelos CAD exportados desde FreeCAD. En Webots se emplearon nodos \texttt{SliderJoint} para los pistones, con restricciones de posición que les permiten alcanzar solo dos estados (retraído/extendido), y nodos \texttt{HingeJoint} para las trampillas de dosificación. El controlador Supervisor, escrito en Python, ejecuta una FSM que refleja los estados del proceso y recibe comandos a través de MQTT. Las ecuaciones de control inverso permiten que, bajo una presión neumática virtual variable, la velocidad del pistón se comporte como \(v \propto 1/P_{\text{virtual}}\), emulando el comportamiento dinámico real.
...
\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA DE FLUJO DE COMUNICACIÓN MQTT – FLUJO DE TELEMETRÍA (VERDE) Y FLUJO DE MANIOBRA (NARANJA)]}
\caption{Flujo de comunicación con diferenciación de telemetría y comandos.}
\label{fig:flujo_mqtt}
\end{figure}

\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA DE CICLO DE VIDA DE LA INFORMACIÓN – ADQUISICIÓN → FILTROS → TRANSFORMACIONES → ALMACENAMIENTO CSV]}
...
\begin{figure}[htbp]
\centering
\textbf{[DIAGRAMA GENERAL DEL SISTEMA – ARQUITECTURA COMPLETA CON CAPAS FÍSICA, RED Y SISTEMA]}
...
\begin{figure}[htbp]
\centering
\textbf{[IMAGEN DE LA MÁQUINA REAL CON ACTUADORES IDENTIFICADOS]} \hfill \textbf{[RENDERIZADO EN WEBOTS CON RESTRICCIONES MECÁNICAS EQUIVALENTES]}
\caption{Comparativa visual entre hardware real y gemelo digital.}
\label{fig:comp_visual}
\end{figure}
...
La métrica de referencia en la industria es la Efectividad Global del Equipo (OEE, por sus siglas en inglés), definida como el producto de la disponibilidad, el rendimiento y la calidad según la norma ISO 22400-2 \cite{iso22400-2}. La literatura especializada señala que el OEE es una herramienta clave para medir el desempeño de los equipos de manufactura \cite{muchiri2008oee}. Dado que en este proyecto no se dispone de celdas de carga ni de sistemas de visión artificial para medir la calidad del producto dosificado, se adaptó el OEE eliminando el factor de calidad, obteniendo así la \textbf{Efectividad Mecánica Operativa (EMO)}:
...
Kit de soldadura y herramientas (depreciación) & Global & 30.00 & 30.00 \\
\hline
\textbf{TOTAL} & & & \textbf{243.20} \\
\hline
\end{longtable}

\section*{Verificación de Citaciones}

\begin{verbatim}
=== VERIFICACIÓN DE CITAS ===

[1] ISO 23247-1:2021. Automation systems and integration — Digital twin 
    framework for manufacturing — Part 1: Overview and general principles.
    International Organization for Standardization.
    (Norma internacional, sin DOI. Disponible en www.iso.org)

[2] ISO 23247-2:2021. Automation systems and integration — Digital twin
    framework for manufacturing — Part 2: Reference architecture.
    International Organization for Standardization.
    (Norma internacional, sin DOI. Disponible en www.iso.org)

[3] ISO 22400-2:2014. Automation systems and integration — Key performance
    indicators (KPIs) for manufacturing operations management — Part 2:
    Definitions and descriptions.
    International Organization for Standardization.
    (Norma internacional, sin DOI. Disponible en www.iso.org)

[4] Michel, O. (2004). Webots: Professional mobile robot simulation.
    International Journal of Advanced Robotic Systems, 1(1), 39-42.
    DOI: 10.5772/5618
    → Verificar: https://doi.org/10.5772/5618

[5] Retrofitting legacy systems for Industry 4.0 via OPC UA and 
    distributed control. (2025). Manufacturing Letters, 46, 12-17.
    DOI: 10.1016/j.mfglet.2025.06.153
    → Verificar: https://doi.org/10.1016/j.mfglet.2025.06.153

[6] Muchiri, P., & Pintelon, L. (2008). Performance measurement using
    overall equipment effectiveness (OEE): literature review and practical
    application discussion. International Journal of Production Research,
    46(13), 3517-3535.
    DOI: 10.1080/00207540601142645
    → Verificar: https://doi.org/10.1080/00207540601142645

[7] OASIS (2014). MQTT Version 3.1.1.
    OASIS Standard.
    (Documento de estándar, sin DOI. Disponible en
    http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/mqtt-v3.1.1.html)

[8] Espressif Systems (2023). ESP32 Series Datasheet v4.5.
    (Documentación técnica oficial, sin DOI. Disponible en
